...
function usage {
    echo "Usage:"
    echo "  $0 SERVICE-ID -f UPF-LB-CONFIG [remaining NF args]"
    echo "  $0 DPDK_ARGS -- ONVM_ARGS -- NF_ARGS"
    echo ""
    echo "Examples:"
    echo "  $0 1 -f path/to/upf_lb.yaml # Launch UPF-LB with service ID 1"
    echo "  UPF_LB_CORE_ID=15 $0 1 -f path/to/upf_lb.yaml"
    exit 1
}

# Require at least 2 arguments: service ID + upf_lb config
if [ "$#" -lt 2 ]; then
    echo "ERROR: Missing required arguments"
    usage
fi

# Set working directory and default UPF-LB path
WORK_DIR=$HOME
DEFAULT_UPF_LB_PATH="$WORK_DIR/L25GC-plus/NFs/onvm-upf/build/5gc/l25gc_upf_lb"

# Default DPDK args
DPDK_BASE_ARGS="-n 3 --proc-type=secondary"
DEFAULT_CORE_ID="${UPF_LB_CORE_ID:-14}"

# Verify the NF binary exists
if [ ! -f "$DEFAULT_UPF_LB_PATH" ]; then
    echo "ERROR: NF executable not found: $DEFAULT_UPF_LB_PATH"
...
# We expect either:
#   0 separators -> simple mode (service ID + upf_lb config)
